...
const AddEmailHistory = (props) => {
    ...
    const fetchRecipientData = async () => {
        if (values.createByContact) {
            console.log(assignToContactData,"assignToContactData check")
            let findEmail = assignToContactData.find((item) => item._id === values.createByContact);
            console.log(findEmail,"findEmail check")
            if (findEmail) {
                setFieldValue('recipient', findEmail.email);
                console.log(findEmail.email,"recipient")
            }
                                ...
                                <>
                                    <GridItem colSpan={{ base: 12, md: 6 }} >
                                        ...
                                        <Flex justifyContent={'space-between'}>
                                            ...
                                            >
                                                {console.log(assignToContactData,"assignToContactData find data")}
                                                {assignToContactData?.map((item) => {
                                                    // return <option value={item._id} key={item._id}>{values.category === 'Contact' ? `${item.email}` : item.leadName}</option>
